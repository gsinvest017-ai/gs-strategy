...
$$R_t = \frac{\sum_{j \in \mathcal{J}} w_{j,t} \cdot \operatorname{sign}\big(W_{j,t}\big)}{\sum_{j \in \mathcal{J}} w_{j,t}}
\;\in [-1, 1], \qquad w_{j,t} = \frac{1}{t}\sum_{s \le t} W_{j,s}^2$$

$|R_t| \to 1$：所選尺度全部同向 → 共振。$R_t \approx 0$：尺度間互相打架。

權重取能量而不是均權，理由是：一個在該標的上幾乎沒有能量的尺度，
它的符號基本上是噪音的符號，不該與主導尺度等權投票。

> **權重必須是擴張視窗，不能是全樣本能量佔比。**
> 本節初稿寫的是 $w_j = \|W_j\|^2 / \sum_i \|W_i\|^2$ ——那是用**整段序列**算出來的常數，
> 於是每一個歷史時點的 $R_t$ 都用到了未來的能量分布。這正是 §0 硬規則禁止的洩漏，
> 而且它特別陰險：權重只影響加權平均、不影響符號，所以洩漏不會讓回測明顯異常，
> 只會讓它穩定地好一點。
> 實作（`cross_scale_resonance`）用的是 $w_{j,t} = \operatorname{mean}(W_{j,s}^2 : s \le t)$，
> 只看 $t$ 以前的資料，逐棒可重現。樣本前期它是同一個量的雜訊估計——
> 那是因果性的誠實代價，不是 bug。**上式已更正為實作的版本。**
